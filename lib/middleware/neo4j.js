...
var map = function(nodeType, typeModels) {
  ...
  return  _.reduce(labels, function(memo, label) {
    memo[label] = nodeType.registerModel(label, typeModels[label].model);
    if (typeModels[label].children) {
      return _.extend(memo, map(label, typeModels[label].children));
    } else {
      return memo;
    }
  }, {});
...
module.exports = function(req, res, next) {
  ...
  req.Graph = db.Graph;
  req.Transaction = neo4j.Transaction;
  req.models = classes;
  next();
};
