...
var canEdit = require('./middleware/canEdit');
...
exports.map = function(nodeType, typeModels) {
  ...
  return  _.reduce(labels, function(memo, label) {
    ...
    _.each(['get', 'put', 'post', 'del'], function(method) {
      ...
      console.log(label, method, memo[label])
      router[method]('/:uid', canEdit, memo[label][method].bind(memo[label]));
    });
    router.post('/', canEdit, memo[label].post.bind(memo[label]));
